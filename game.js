(() => {
  ...
  function startGameSolo() {
    ...
    resetGameData();
    $('mpScoreBoard').style.display = 'none';
    $('formulaCard').style.display = 'block'; $('catalogCard').style.display = 'block';
    setScreen('game'); nextRound();
  }
  ...
  function nextRound() {
    ...
    $('hintBtn').disabled = isMultiplayer;
  ...
  $('soundToggle').addEventListener('click', () => {
    if (audio.volume > 0) {
      ...
      let v = lastVolume > 0 ? lastVolume : 1;
      audio.volume = v; $('volumeSlider').value = v; $('soundToggle').textContent = v < 0.5 ? '🔉' : '🔊';
    }
  });
